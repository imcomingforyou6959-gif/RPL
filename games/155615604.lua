...
run(function()
    local GunTracers = require(replicatedStorageService:WaitForChild("SharedModules"):WaitForChild("GunTracers"))
    if not GunTracers then
        notif('Teleport Bullets', 'GunTracers module not found', 3, 'alert')
        return
    end

    local teleportEnabled = false
    local teleportRange = 150
    local teleportPart = "Head"
    local oldCastRay
    local hookedTable = nil
    ...
    local function findTarget(origin)
        ...
        if ent and ent[teleportPart] then
            return ent[teleportPart]
        end
        return nil
    end

    local function setupHook()
        if oldCastRay then return end

        for _, v in pairs(getgc(true)) do
            if type(v) == "table" and rawget(v, "createBullet") and rawget(v, "createSniper") and rawget(v, "createTaser") then
                local castRayFunc = rawget(v, "castRay")
                if castRayFunc and type(castRayFunc) == "function" then
                    oldCastRay = castRayFunc
                    hookedTable = v
                    v.castRay = function(...)
                        if teleportEnabled then
                            local args = {...}
                            local origin = args[2]
                            local targetPart = findTarget(origin)
                            if targetPart then
        ...
        if not oldCastRay then
            notif('Teleport Bullets', 'castRay function not found', 3, 'alert')
        end
    end

    local TeleportModule = vape.Categories.Combat:CreateModule({
        Name = "Teleport Bullets",
        Function = function(callback)
            teleportEnabled = callback
            if callback then
                setupHook()
            end
        end,
        Tooltip = "Teleports bullets directly to the nearest enemy"
    })

    TeleportModule:CreateSlider({
        Name = "Range",
        Min = 50,
        Max = 1000,
        Default = 150,
        Function = function(v) teleportRange = v end,
        Suffix = function(v) return v == 1 and "stud" or "studs" end
    })
    TeleportModule:CreateDropdown({
        Name = "Target Part",
        List = {"Head", "HumanoidRootPart", "UpperTorso"},
        Default = "Head",
        Function = function(v) teleportPart = v end
    })

    vape:Clean(function()
        if oldCastRay and hookedTable then
            hookedTable.castRay = oldCastRay
        end
    end)
end)
